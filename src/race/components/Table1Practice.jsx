...
export const Table1Practice = ({id}) => {

    const {race, races} = useRacesStore();
    const { practice1, setStartPractice1 } = useRankingsStore();

    const cargarQ1 = () => {
        const carrera = races.find(elem => (elem.type === "1st Practice") && (elem.competition.id === race.competition.id));
        setStartPractice1(carrera.id);
    };
